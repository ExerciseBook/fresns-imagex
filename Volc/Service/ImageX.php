...
namespace App\Plugins\ImageX\Volc\Service;

use App\Plugins\ImageX\Volc\Base\V4Curl;
use Exception;
...
{
    protected function getConfig($region)
    {
        switch ($region) {
            case 'cn-north-1':
                $config = [
    ...
    public function upload($uploadHost, $storeInfo, $filePath)
    {
        ...
        $tosClient = new Client([
            'base_url' => "https://" . $uploadHost,
            'timeout' => 5.0,
        ]);

        $request = $tosClient->createRequest('PUT', $storeInfo["StoreUri"], ["body" => $body, "headers" => ['Authorization' => $storeInfo["Auth"], 'Content-CRC32' => $crc32]]);
        $response = $tosClient->send($request);
    ...
    public function getUploadAuth(array $serviceIDList, $expire = 3600, $keyPtn = '')
    {
        $applyRes = [];
        $commitRes = [];
    ...
    public function deleteImages($serviceID, array $uris = [])
    {
        $response = $this->request('DeleteImageUploadFiles', ['query' => ['ServiceId' => $serviceID], 'json' => ['StoreUris' => $uris]]);
        return (string)$response->getBody();
    }

    public function requestImageX($action, array $config = [])
    {
